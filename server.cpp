...
void    Server::addClient()
{
   ...
   socklen_t clientAddrLen = sizeof(clientAddr);
    int client_socket = accept(serverSocket_, (sockaddr *)&clientAddr, &clientAddrLen); // fd for new client
    if (clientSocket_ == -1) { // tu utilises le meme dans la fonction.... a voir
        std::cerr << "Error accepting client" << std::endl;
        // enlever continue par cd que on est plus dans un looop;
    }
    
    clients_.push_back(Client(client_socket));
    pollfd clientPollfd = {client_socket, POLLIN, 0};
    fds_.push_back(clientPollfd);    
...
void Server::start()
{
        pollfd serverPollfd = { serverSocket_, POLLIN, 0};
    //  int serverSocket, clientSocket;
   clientAddrLen_ = sizeof(clientAddr_);
